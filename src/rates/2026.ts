...
function full(totalRate: number, employeeRate: number): IncomeBracket {
  return {
    ...
    compute: (ow, aw) => {
      const tw = D(ow).plus(aw);
      return {
        total: D(totalRate).times(tw),
        employee: D(employeeRate).times(tw),
      };
    },
  };
}
