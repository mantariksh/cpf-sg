...
import { computeAllocation, computeContributions } from './index.js';
...
async function run(): Promise<void> {
  ...
  const birthMonth = parseInt(
    await input({ message: 'Birth month (1–12):' }),
    10,
  );
  const contributionYear = parseInt(
    await input({
      message: 'Contribution year:',
      default: String(new Date().getFullYear()),
    }),
    10,
  );
  const contributionMonth = parseInt(
    await input({
      message: 'Contribution month (1–12):',
  ...
  const aw = parseFloat(
    await input({ message: 'Additional Wages ($):', default: '0' }),
  );
  const owCsv = await input({
    message:
      'OW for other months this year, excl. current month (comma-separated, or blank):',
    default: '',
  });
  ...
  const residencyStatus = await select<ResidencyStatus>({
    ...
    choices: [
      ...
      {
        value: 'SPR_1FG',
        name: 'SPR_1FG — SPR 1st year, Full employer/Graduated employee',
      },
      {
        value: 'SPR_2FG',
        name: 'SPR_2FG — SPR 2nd year, Full employer/Graduated employee',
      },
    ],
  });
  ...
  console.log(
    `  Ordinary Account:      $${fmt(allocation.ordinaryAccount, 2)}`,
  );
  if (cpfAge <= 55) {
    console.log(
      `  Special Account:       $${fmt(allocation.specialAccount, 2)}`,
    );
  } else {
    console.log(
      `  Retirement Account:    $${fmt(allocation.retirementAccount, 2)}`,
    );
  }
  console.log(
    `  MediSave Account:      $${fmt(allocation.medisaveAccount, 2)}`,
  );
  console.log();
}
